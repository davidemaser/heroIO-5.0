...
$(function () {
  $(app.objects.bo).on('click', '.btnAdd', () => {
    ...
    $(app.messages.overlay).css('display', 'none')
  ...
  }).on('click', app.buttons.mode, () => {
    let a = $(app.dom.b).attr('data-nmode');
    if (a === 'hero') {
      $(app.dom.b).attr('data-nmode', 'hello');
      $(app.buttons.mode).attr('data-nmode', 'hello');
      $(app.buttons.mode).html('Switch to Hero Banner Mode');
      Init.switchModes('hello')
    } else if (a === 'hello') {
      $(app.dom.b).attr('data-nmode', 'hero');
      $(app.buttons.mode).attr('data-nmode', 'hero');
      $(app.buttons.mode).html('Switch to Hello Bar Mode');
    ...
    $(app.errors.list).css('display', 'none');
    $(app.objects.r).css('overflow', 'auto');
    ...
    $(app.images.counter).attr('style', '');
    $(app.images.counter).text('Shopify CDN');
    ...
    if (e.keyCode === 13) {
      $(app.selectors.number).trigger("enterKey");
      Items.addMulti($(app.selectors.number).val());
      $(app.selectors.number).parent().parent().parent().parent().hide();
    }
  }).on('keyup', app.selectors.lsl, (e) => {
    if (e.keyCode === 13) {
      $(app.selectors.lsl).trigger("enterKey");
      Json.prepareJSON('save', $(app.selectors.lsl).val());
      $(app.selectors.lsl).parent().parent().parent().parent().hide();
      Init.setHeadSec();
    }
